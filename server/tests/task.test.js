...
describe('Task API integration tests', () => {
  beforeAll(async () => {
    ...
    // Utwórz listę, aby móc przypisać do niej taska
    const listRes = await request(app)
      .post('/list/add')
      .set('Authorization', `Bearer ${token}`)
      .send({ name: 'Lista do testów tasków' });
    ...
    createdListId = lists.body.find(list => list.name === 'Lista do testów tasków')?.id;
  ...
  test('Should fail to add task without token', async () => {
    const res = await request(app)
      .post('/tasks/add')
      .send({ listId: createdListId, description: 'Zadanie bez tokenu' });
  ...
  test('Should add task with valid token', async () => {
    const res = await request(app)
      .post('/tasks/add')
      .set('Authorization', `Bearer ${token}`)
      .send({ listId: createdListId, description: 'Zadanie testowe' });
  ...
  test('Should fail to update task without token', async () => {
    const res = await request(app)
      ...
      .send({ newDescription: 'Nowy opis bez tokenu' });
  ...
  test('Should update task description with token', async () => {
    const res = await request(app)
      .put(`/tasks/edit/${createdTaskId}`)
      .set('Authorization', `Bearer ${token}`)
      .send({ newDescription: 'Nowy opis zadania' });
